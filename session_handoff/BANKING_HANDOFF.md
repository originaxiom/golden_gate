...
(memos 10–25, 2026-08-22, after digesting B1110–B1136): the simultaneous closing →
the fork theorem → the gauge closing → the Y-selection → the installment plan → the
family triplet → the first beat → the descent → Σ-27 → the S₄ torsor → the AW typing → the unit dictionary → the voice ladder → the Weinberg point → the symmetry-point table → B−L the fourth direction. Phase III differs from phase II in kind:
...
| `memos/` | The twenty-five memos of phases II–III, in order below. |
...
21. **`UNIT_DICTIONARY.md`** (phase III) — prime-lane cell PR-2 executed per the ruling:
   for every loxodromic γ, the eigenvalue satisfies the **palindromic integer quartic
   x⁴ − T·x³ + (N+2)·x² − T·x + 1** (T = Tr t, N = N t; constant term 1 ⟹ ALWAYS a
   unit) — verified EXACTLY on all 672 loxodromic classes to word-length 8 (122 (T,N)
   labels); **ℓ(γ) = log Mahler(P_{T,N})** verified to 3.8×10⁻¹⁵ across the census (the
...
23. **`WEINBERG_POINT.md`** (phase III) — the closing's FIRST NUMBER, preregistered in
   the certificate header before the run: across all 9 generic gauge closings × both
   selected Y's × both kept su(2)'s (72 combinations), **sin²θ_W = 3/8 at the symmetry
   point in every one — Tr T₃² = 3, Tr Q² = 8 exactly — closing-independent, and 36/72
   assignments simultaneously emit the physical electric-charge multiset of the 27**
   ({±1:2, ±2/3:3, ∓1/3:6, 0:5}) with zero embedding freedom. 3/8 is the textbook
...
25. **`BL_FOURTH_DIRECTION.md`** (phase III, masterplan Lane A: SP-1/SP-3/SP-4) —
   **SP-1**: B−L solved from FORCED targets only (Q_L, u^c, e^c): solvable with exactly
   one free parameter = the textbook d^c↔D̄ swap; on the physical branch B−L is physical
   on all 27 (exotic D forced to −2/3 unimposed), traceless, and **provably outside
   span{Y, T₃L, T₃R} — an independent fourth Cartan direction**, explaining memo 24's
   Pati–Salam negative; the non-color charge space = exactly {T₃L,T₃R,Y,B−L}. **SP-3**:
   the (color,T₃L,Y,Q) table is ONE multiset across all 36 physical assignments, 36/36
   anomaly-zero. **SP-4**: the E₈ four-slot stabilizer's index-2 element = the antipode
   w₀ = −1, pattern OOOO (verified; no mixed pattern exists). Certificates:
   `certificates/sp1_bl.py`, `sp1b.py`, `sp4_idx2.py`, `sp4b.py`; outputs `outputs/sp1*_out.txt`, `sp4b_out.txt`.
